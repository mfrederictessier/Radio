...
var map = L.map('map', {
    ...
    maxZoom: 5, // Définir le zoom maximum
    maxBounds: [
        [-85, -179.5], // Sud-Ouest
        [85, 179.5]    // Nord-Est
    ]
...
regions.forEach(function(region) {
    ...
    if (stations.length > 0) {
        ...
        // Calculer le rayon du cercle en fonction du nombre de stations
        var radius = 50000; // Exemple : 1000 mètres par station
        
        // Créer le cercle de couverture
        L.circle(region.coords, {
            radius: radius,
            weight: 1,
            color: 'yellowgreen', // Couleur du contour du cercle
            fillColor: 'yellowgreen', // Couleur de remplissage du cercle
            fillOpacity: 0.3 // Opacité du remplissage (0 = transparent, 1 = opaque)
        }).addTo(map);
        // Créer le cercle de couverture
        L.circle(region.coords, {
            radius: 100000,
            ...
            fillOpacity: 0.1 // Opacité du remplissage (0 = transparent, 1 = opaque)
        }).addTo(map);
        ...
        var popupOpen = false; // Variable pour suivre l'état du popup
        ...
        marker.on('mouseout', function() {
            // Fermer le popup lorsque la souris quitte le marqueur
            if (!popupOpen) {
                map.closePopup();
            }
        });
        marker.on('mouseover', function() {
            // Ouvrir le popup au survol si le popup n'est pas déjà ouvert
            if (!popupOpen) {
                L.popup()
        ...
        marker.on('popupopen', function() {
            popupOpen = true; // Mettre à jour l'état du popup à ouvert
        });

        marker.on('popupclose', function() {
            popupOpen = false; // Mettre à jour l'état du popup à fermé
        });
